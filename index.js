...
app.post('/analyze', async (req, res) => {
  ...
  const allScores = results.flatMap(r =>
    (r.result || []).map(x => x.score).filter(s => typeof s === 'number')
  );
  ...
  console.log(`✅ Analysis complete. Total files: ${results.length}, Score: ${overallQualityScore}`);
